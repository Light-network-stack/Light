...
do
	...
	/usr/bin/light_srv -c $mask -n 4 -d librte_pmd_ixgbe.so  --proc-type=auto  -- -p 0x3 -l $log_level --num-procs=$1 --proc-id=$proc_id >out$proc_id.file 2>&1 &
	sleep 10
done
echo "bingo"
