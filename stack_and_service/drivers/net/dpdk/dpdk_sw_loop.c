...
printf("%s %d\n",__FILE__,__LINE__);
	...
	if (rte_ring_enqueue_burst(loop_ring[priv->port_number],&skb,1) == 0) {
		printf("Warning: In lpbkdpdk_xmit_frame, the return value of rte_ring_enqueue_burst is 0\n");
	}
	transmitted++;
	return NETDEV_TX_OK;
}
...
static void rx_construct_skb_and_submit(struct net_device *netdev)
{
	...
	if (ret == 0) {
		printf("In rx_construct_skb_and_submit, rte_ring_dequeue_burst(loop_ring[priv->port_number],(void **)skbs,size) = 0\n");
	}
