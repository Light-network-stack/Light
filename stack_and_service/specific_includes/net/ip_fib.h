...
#ifdef ARP_DEBUG
	printf("table->tb_id = %u\n", table->tb_id);
	printf("table->tb_default = %d\n", table->tb_default);
	printf("table->tb_data[0] = %u\n", table->tb_data[0]);

	struct fib_table *ft;
	struct hlist_head *hhead = &table->tb_hlist;
	printf("@ show table\n");
	hlist_for_each_entry_rcu(ft, hhead, tb_hlist) {
		printf("ft->tb_id = %u\n", ft->tb_id);
		printf("ft->tb_default = %d\n", ft->tb_default);
		printf("ft->tb_data[0] = %u\n", ft->tb_data[0]);
	}
	printf("@ show table (END)\n");
#endif	
	ret = fib_table_lookup(table, flp, res, FIB_LOOKUP_NOREF);
#ifdef ARP_DEBUG
	printf("RT_TABLE_LOCAL, ret = %d\n", ret);
#endif
	if (!ret)
		return 0;
