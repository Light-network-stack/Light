...
{
#ifdef IP_INPUT_DEBUG
	printf("start ip_call_ra_chain\n");
#endif
...
{
#ifdef IP_INPUT_DEBUG
	printf("start ip_local_deliver_finish\n");
#endif
...
int ip_rcv(struct sk_buff *skb, struct net_device *dev, struct packet_type *pt, struct net_device *orig_dev)
{
	...
	BUILD_BUG_ON(IPSTATS_MIB_ECT1PKTS != IPSTATS_MIB_NOECTPKTS + INET_ECN_ECT_1); //nothing
	...
	if (unlikely(ip_fast_csum((u8 *)iph, iph->ihl))) //ip checksum
		goto csum_error;
...
#ifdef IP_INPUT_DEBUG
		printf("IP_INC_STATS_BH(dev_net(dev), IPSTATS_MIB_INTRUNCATEDPKTS)\n");
#endif
		goto drop;
	}
	...
	if (pskb_trim_rcsum(skb, len))
	{
		IP_INC_STATS_BH(dev_net(dev), IPSTATS_MIB_INDISCARDS);
#ifdef IP_INPUT_DEBUG
		printf("IP_INC_STATS_BH(dev_net(dev), IPSTATS_MIB_INDISCARDS)\n");
#endif
		goto drop;
	}
