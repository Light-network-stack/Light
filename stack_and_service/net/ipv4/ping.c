...
next_port:
	...
	if (sk_unhashed(sk)) {
		...
		#ifdef CLOSE_DEBUG
		printf("In ping_get_port, sock_hold(sk)\n");
		#endif
		sock_hold(sk);
...
void ping_unhash(struct sock *sk)
{
	...
	if (sk_hashed(sk)) {
		...
		#ifdef CLOSE_DEBUG
		printf("In ping_unhash, sock_put(sk)\n");
		#endif
...
static struct sock *ping_lookup(struct net *net, struct sk_buff *skb, u16 ident)
{
	...
	ping_portaddr_for_each_entry(sk, hnode, hslot) {
		...
		#ifdef CLOSE_DEBUG
		printf("In ping_lookup, sock_hold(sk)\n");
		#endif
		sock_hold(sk);
		goto exit;
	}
...
out:
	#ifdef CLOSE_DEBUG
	printf("In ping_err, sock_put(sk)\n");
	#endif
...
void ping_rcv(struct sk_buff *skb)
{
	...
	if (sk != NULL) {
		...
		#ifdef CLOSE_DEBUG
		printf("In ping_rcv, sock_put(sk)\n");
		#endif
