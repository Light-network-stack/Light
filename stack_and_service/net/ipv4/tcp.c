 ...
 */
//turn on or turn off the debug info
// #define TCP_DEBUG
...
#if 0
//sztly#ifndef OPTIMIZE_SENDPAGES
    while (size > 0)
    {
        bool can_coalesce;
...
wait_for_memory:
    ...
    #ifdef TCP_DEBUG
//    printf("do_tcp_sendpage()\n");
    #endif
//        copy = min_t(int, size_goal,size);
    #ifdef TCP_DEBUG
//sztly    printf("size_goal %d mss_now %d\n",size_goal,mss_now);
    #endif
    copy = size_goal;
//  copy = min_t(int, copy, mss_now);
    ...
    // #ifdef TCP_DEBUG
    printf("before user_get_buffer, copy = %d\n", copy);
    // #endif
    #endif

    if (unlikely(((mbuf = user_get_buffer(sk, &copy)) == NULL))) // copy = 1448
    {
        #ifdef LIGHT_RTE_RING_DEBUG
        printf("Error: in do_tcp_sendpages, (mbuf = user_get_buffer(sk, &copy)) == NULL\n");
        #endif
    ...
    #ifdef LIGHT_RTE_RING_DEBUG
    // #ifdef TCP_DEBUG
    printf("after user_get_buffer, copy = %d\n", copy);
    // #endif
    #endif
    ...
    while (mbuf)
    {
        ...
        if (!skb)
        {
            printf("Error: sk_stream_alloc_skb(sk, 0, sk->sk_allocation) = NULL\n");
...
adjudge_to_death:
    // printf("  after adjudge_to_death\n");
    state = sk->sk_state;
    #ifdef CLOSE_DEBUG
    printf("In tcp_close, sock_hold(sk)\n");
    #endif
    sock_hold(sk);
    sock_orphan(sk);
...
out:
    ...
    #ifdef CLOSE_DEBUG
    printf("In tcp_close, sock_put(sk)\n");
    #endif
...
{
    printf("In set_thash_entries, str = %s\n", str);
...
void __init tcp_init(void)
{
     ...
     */
    thash_entries = 1 << 24; // otherwise thash_entries is not initialized.
    tcp_hashinfo.ehash =
    ...
    printf("In tcp_init, thash_entries = %u, tcp_hashinfo.ehash_mask = %u, &tcp_hashinfo.ehash_mask = %x\n", thash_entries, tcp_hashinfo.ehash_mask, &tcp_hashinfo.ehash_mask);
