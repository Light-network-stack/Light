...
{
#ifdef TCP_INPUT_DEBUG
	printf("start tcp_rcv_state_process\n");
#endif
	struct tcp_sock *tp = tcp_sk(sk);
	struct inet_connection_sock *icsk = inet_csk(sk);
	...
	case TCP_LISTEN:
		...
		if (th->syn)
		{
			...
			{
#ifdef TCP_INPUT_DEBUG
				printf("icsk->icsk_af_ops->conn_request(sk, skb) < 0\n");
#endif
				return 1;
			}
